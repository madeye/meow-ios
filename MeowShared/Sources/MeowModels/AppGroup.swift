...
public enum AppGroup {
    ...
    /// (`meow_config_cn_bypass_probe`): a probe verdict + the CN CIDR set,
    /// keyed by the SHA-256 of the config it was probed from. The extension
    /// reads it at tunnel start to exclude CN routes from the TUN, ignoring
    /// it whenever the hash no longer matches `configURL`. Keep in sync with
    /// `MWAppGroup.cnBypassURL` (ObjC).
    public static var cnBypassURL: URL {
        containerURL.appending(path: "cn-bypass.txt")
    }
    ...
    public static func configureBackup() {
        ...
        // Derived artifact, regenerated by the pre-connect probe.
        setBackupExclusion(cnBypassURL, excluded: true)
