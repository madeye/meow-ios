//! DNS-over-HTTPS client. Ported from the Android FFI: each request is sent
//! through the local SOCKS5 mixed listener via `reqwest` with a
//! `socks5h://127.0.0.1:<mixed-port>` proxy. That keeps DNS subject to the
//! same rule/proxy-group decisions as regular TCP traffic and avoids the
//! circular-DNS problem (resolving the DoH server by DNS before the tunnel is
//! up).
//!
//! The request path is `reqwest (h2/h1) → socks5h → MixedListener → mihomo
//! adapter → upstream DoH`. No TLS / hyper plumbing lives in this crate
//! anymore; reqwest's rustls-tls feature handles that.
...
pub fn init_doh_client(socks_port: u16) {
    DOH_CLIENT.get_or_init(|| {
        ...
        // The DoH path traverses whichever upstream proxy the user configured
        // through mihomo, matching the Android reference; accept any server
        // cert. DNS payload is not a secret and we trust the operator.
